...
export const generateProductTemplate = (variant = 'full') => {
  ...
  //
  // Thirteen columns, as before the sheet was widened. The importer still reads
  // the wider set — Arabic name, SKU, minimum order quantity, on-site fitting —
  // so a file that already has those columns keeps importing them; they are
  // simply no longer part of the blank sheet handed to a new seller. The export
  // in productExport.js writes the full set, which is what makes a downloaded
  // catalogue round-trip without losing its Arabic.
  const templateData = [
    {
  ...
  const instructions = [
    ...
    // Reads NO, not the old YES: the importer accepts a blank price and lands
    // the product at 0, out of stock. Restoring the old wording here would be
    // documenting a rule the code stopped enforcing.
