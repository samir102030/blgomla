...
const AdminEditNews: React.FC = () => {
  ...
  const loadNews = async (newsId: string) => {
    try {
      ...
      const newsItem = await newsAPI.getByIdAdmin(newsId);
      
      // Store original data for change tracking
      setOriginalData(newsItem);
      
      // Set English values or fallback to first available language
      setTitle(newsItem.title?.en || newsItem.title || '');
      setContent(newsItem.content?.en || newsItem.content || '');
      
      // Initialize multilingual data with full structure (only in edit mode)
      if (isEdit) {
        initializeFromData(newsItem, ['title', 'content']);
      }
  ...
  const validateForm = (): boolean => {
    ...
    if (!isEdit && !image && !imagePreview) {
      errors.image = 'News image is required';
    }
    ...
    try {
      ...
      if (isEdit && id) {
        ...
        if (Object.keys(updateData).length > 0) {
          await newsAPI.update(id, updateData);
          toast({
            title: 'Success',
        ...
        // For new news, upload image and send all data
        let imageUrl = '';
        if (image) {
          const uploadResponse = await uploadAPI.uploadImage(image);
  ...
  return (
    <Box p={10}>
          ...
          <VStack spacing={16} align="stretch">
            {/* News Title and Image - Side by Side */}
                ...
                <Box flex="1" position="relative">
                  <Flex align="center" h="64px" borderRadius="lg" border={`2px solid ${validationErrors.image ? '#E53E3E' : '#EAEAEA'}`} bg="#fff" pl={3} pr={0} py={3}>
                    <Text fontFamily="IBM Plex Sans" fontSize="24px" color="#8D8D8D" flex="1">
                      {image ? image.name : imagePreview ? 'Current image' : 'upload news image'}
                    </Text>
                    <Box
                      bg="#EAEAEA"
                      h="64px"
            ...
            <AdminActionButtons
              ...
              saveLabel={isEdit ? 'Update News' : 'Save News'}
              isLoading={saving}
              isDisabled={saving}
            />
          </VStack>
