...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ collapsed }) => {
  ...
  return (
    <div
      className={`bg-[#002B5B] shadow-lg transition-all duration-300 ${
        collapsed ? "w-16" : "w-64"
      } flex flex-col`}
    >
      {/* Logo */}
      <div className="p-4 border-b border-[#FFD600]/20">
        <div className="flex items-center">
          ...
          {!collapsed && (
            <span className="ml-3 text-xl font-semibold text-[#FFD600]">
              ELGOMLA
            </span>
          )}
        </div>
      ...
      <nav className="flex-1 p-4 space-y-2">
        {menuItems
          .filter((item) => {
            // If the current user is a store, hide Vendors and User entries
            if (
              user?.role === "store" &&
              (item.name === "Vendors" || item.name === "User")
            ) {
              return false;
            }
            return true;
