...
interface AdminArticleDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  article: {
    image: string;
...
const AdminArticleDetailsModal: React.FC<AdminArticleDetailsModalProps> = ({ isOpen, onClose, article }) => {
      ...
      <ModalContent borderRadius="2xl" p={0} maxW="700px" bg="#fff">
        <Box px={8} pt={10} pb={0}>
          ...
          <Flex align="center" justify="space-between" mb={12}>
            <Text fontFamily="IBM Plex Sans" fontSize="32px" fontWeight={500} color="#000000e5" lineHeight="56px">Article details</Text>
          ...
          {/* Hero Section */}
          <Box mb={10}>
            <Image src={article.image} alt="Article banner" borderRadius="lg" w="100%" h="324px" objectFit="cover" mb={6} bg="#EEE" />
            <Text fontFamily="IBM Plex Sans" fontSize="24px" color="#424242" fontWeight={500} mb={2}>{article.title}</Text>
            <Text fontFamily="IBM Plex Sans" fontSize="20px" color="#717171" fontWeight={400} mb={2}>{article.content}</Text>
          </Box>
...
export default AdminArticleDetailsModal; 