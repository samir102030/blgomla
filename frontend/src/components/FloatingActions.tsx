...
const FloatingActions: React.FC = () => {
  ...
  return (
    <div
      className="fixed bottom-5 left-4 sm:bottom-6 sm:left-6 z-30 flex flex-col-reverse items-start gap-3 pointer-events-none"
      aria-hidden={false}
    >
      ...
      <a
        ...
        className="pointer-events-auto group relative inline-flex items-center justify-center w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#25D366] text-white shadow-lg shadow-[#25D366]/30 hover:shadow-xl hover:shadow-[#25D366]/40 hover:scale-105 active:scale-95 transition-all"
      >
      ...
      {/* Back-to-top — visible only after scroll */}
      <button
        type="button"
        onClick={scrollTop}
        aria-label={t("Back to top")}
        title={t("Back to top")}
        className={`pointer-events-auto inline-flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-full bg-[var(--surface)] border border-[var(--border)] text-[var(--text)] shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all ${
          showTop
